...
def edges_grid_rook(nrows, ncols):
    ...
    def nid(r, c): return r * ncols + c
    E = []
    for r in range(nrows):
        for c in range(ncols):
            if r+1 < nrows: E.append((nid(r,c), nid(r+1,c)))
            if c+1 < ncols: E.append((nid(r,c), nid(r,c+1)))
    return E
...
def test_segregated_vs_swapped_block_k2(grid8x8_block):
    ...
    s_swap = S(df_sw, grid8x8_block, "y", k=2, permutations=64, seed=42,
               keep_sim=True, verbose=False)
...
def test_variability_with_more_bins_or_sparser_graph(grid8x8_rook):
    ...
    s = S(df, grid8x8_rook, "y", k=4, permutations=199, seed=123,
          keep_sim=True, verbose=False)
...
def test_seed_determinism_and_pvalue_ties(grid8x8_block):
    ...
    s1 = S(df, grid8x8_block, "y", k=2, permutations=99, seed=7, keep_sim=True, verbose=False)
    s2 = S(df, grid8x8_block, "y", k=2, permutations=99, seed=7, keep_sim=True, verbose=False)
...
def test_repr_includes_key_fields(grid8x8_rook):
    ...
    text = repr(s)
    for token in ["S Spatial Polarization Summary", "Variable:", "S:", "Number of spatial components:"]:
